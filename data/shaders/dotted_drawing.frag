...
void main (void)
{
    ...
    vec3 col = texture2D(tex, q).rgb;
   
    vec2 r = random(q);
    r.x *= PI2;
    vec2 cr = vec2(sin(r.x),cos(r.x))*sqrt(r.y);
    
    vec3 blurred = texture2D(tex, q + cr * (vec2(mSize) / iResolution.xy) ).rgb;
    ...
    //if (iMouse.z > 0.5) 
    {
        ...
        for (int i = -kSize; i <= kSize; ++i) {
            for (int j = -kSize; j <= kSize; ++j) {
                blurred += kernel[kSize+j]*kernel[kSize+i]*texture2D(tex, (texCoord+vec2(float(i),float(j))) / iResolution.xy).rgb;
            }
    	}
   		blurred = blurred / Z / Z;
    }
